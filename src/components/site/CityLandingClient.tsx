...
export default function CityLandingClient({ page, images, districts, faq, relatedServices, lang, trUrl, enUrl }: CityLandingClientProps) {
    ...
    return (
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <nav className="mb-8 flex items-center gap-2 text-sm text-black/50">
                ...
                <Link href={servicesHref} className="hover:text-black transition uppercase tracking-widest text-[10px] font-bold">
            ...
            {districts.length > 0 && (
                <div className="mb-20">
                    <div className="flex items-center gap-4 mb-8">
                        <h2 className="text-xs uppercase tracking-[0.3em] text-black/40 font-semibold">
                            {t("Hizmet Bölgelerimiz", "Our Service Areas")}
                        </h2>
                        <div className="h-[1px] flex-1 bg-black/5" />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {districts.map((d, i) => (
                            <div key={i} className="p-6 border border-black/10 rounded-2xl bg-black/[0.01]">
                                <h3 className="text-base font-bold text-black uppercase tracking-tight mb-2">{d.title}</h3>
                                <p className="text-sm text-black/60 leading-relaxed">{d.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {faq.length > 0 && (
                <div className="mb-20">
                    ...
                    <div className="space-y-3">
                        {faq.map((item, i) => (
                            <details key={i} className="group border border-black/10 rounded-2xl overflow-hidden bg-black/[0.01]">
                                <summary className="flex items-center justify-between px-6 py-5 cursor-pointer list-none">
                                    <span className="text-black font-medium text-sm pr-4">{item.q}</span>
                                    <span className="text-black/30 group-open:rotate-45 transition-transform duration-200 text-2xl leading-none shrink-0">+</span>
                                </summary>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div>
                        ...
                        <p className="text-black/60 mb-8 leading-relaxed">
                            {t(
                                "Bu bölgeye özel etkinlik teknolojisi çözümleri için bizimle iletişime geçebilirsiniz.",
                                "Contact us for event technology solutions tailored to this area."
                            )}
                        </p>
